...
function checkBrowserFeatures() {
    ...
    // Disable rule because Riot runs without (on old Edge Versions)
...
function applyPassedParameters() {
    ...
    if (userId && homeserverUrl && accessToken) {
        ...
        console.error("All three authentication properties have to be set: " +
            "matrix-user-id, matrix-homeserver-url, matrix-access-token");
    }

    if (getDatasetConfig("matrixRoomId")) {
        localStorage.setItem("mx_last_room_id", getDatasetConfig("matrixRoomId"));
    }

    if (getDatasetConfig("vectorDefaultToggled")) {
        const toggled = getDatasetConfig("vectorDefaultToggled") === "true";
    ...
    if (getDatasetConfig("vectorForceToggled")) {
        const toggled = getDatasetConfig("vectorForceToggled") === "true";
        // only set default if not already set
        localStorage.setItem("mx_room_toggled", toggled ? "true" : "false");
        localStorage.setItem("mx_menu_toggled", toggled ? "true" : "false");
    }
...
async function start() {
    ...
    try {
        ...
        const preventRedirect = true; // fragparts.params.client_secret || fragparts.location.length > 0;
        ...
        // passed parameters
        applyPassedParameters();

        const loadOlmPromise = loadOlm(getPublicPath());
        // set the platform for react sdk
        preparePlatform();
        // load config requires the platform to be ready
        const loadConfigPromise = loadConfig(getDatasetConfig('vectorConfig'));
        ...
        const loadLanguagePromise = loadLanguage(getDatasetConfig('matrixLang'));
